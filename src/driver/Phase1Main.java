...
/*
 * Modified by Yu Xi, Xiaoyu Yuan, Xinyue Duan
 */
...
public class Phase1Main {
    ...
    private static void testTradables() throws InvalidValueException {
        ...
        /*This is the test method we add  for invalid buy side quote */
        try {
            System.out.println("9) Attempt to create a quote using INVALID data (negative buy volume) - should throw an exception:");
            quote1 = new Quote("USER2", "GE", PriceFactory.makeLimitPrice("$21.56"), -100, PriceFactory.makeLimitPrice("$21.62"), 100);
            System.out.println("If this prints then you have accepted invalid data in your Quote - ERROR!");
        } catch (Exception e) { // Catch anything you throw.
            System.out.println("Properly handled an invalid volume -- error message is: " + e.getMessage() + "\n");
    ...
    /*we add more testing case for this method*/
    ...
    private static void verifyMathematicalOperations() {
        ...
        try{
        	testPriceHolder.get(0).add( testPriceHolder.get(8) );
        	System.out.println(("   FAIL: Adding a MARKET price to a LIMIT Price succeeded: (" + testPriceHolder.get(0) + " + " + testPriceHolder.get(8) + ")"));
        }
        catch(InvalidPriceOperation ex){
        	System.out.println("   PASS: " + ex.getMessage() + ": (" + testPriceHolder.get(0) + " + " + testPriceHolder.get(8) + ")");
            
        }
        try{
        	testPriceHolder.get(0).add( null );
        	System.out.println(("   FAIL: Adding a null price to a LIMIT Price succeeded: (" + testPriceHolder.get(0) + " + " + null + ")"));
        }
        catch(InvalidPriceOperation ex){
        	System.out.println("   PASS: " + ex.getMessage() + ": (" + testPriceHolder.get(0) + " + " + null + ")");
            
        }
        
        try{
        	testPriceHolder.get(0).subtract( testPriceHolder.get(8) );
        	System.out.println(("   FAIL: Subtracting a MARKET price from a LIMIT Price succeeded: (" + testPriceHolder.get(0) + " + " + testPriceHolder.get(8) + ")"));
        }
        catch(InvalidPriceOperation ex){
        	System.out.println("   PASS: " + ex.getMessage() + ": (" + testPriceHolder.get(0) + " + " + testPriceHolder.get(8) + ")");
        ...
        try{
        	testPriceHolder.get(0).add( null );
        	System.out.println(("   FAIL: Subtractin a NULL price from a LIMIT Price succeeded: (" + testPriceHolder.get(0) + " + " + null + ")"));
        }
        catch(InvalidPriceOperation ex){
        	System.out.println("   PASS: " + ex.getMessage() + ": (" + testPriceHolder.get(0) + " + " + null + ")");
